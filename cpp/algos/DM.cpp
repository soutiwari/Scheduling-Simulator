...
using namespace std;
struct process{
    int pno,arrival,relativedeadline,execution,period,absolutedeadline;
    bool flag;
...
int main(int arg, char* argv[])
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        file>>p[i].period;
        file>>p[i].execution;
        file>>p[i].relativedeadline;
        //cin>>p[i].pno>>p[i].arrival>>p[i].period>>p[i].execution>>p[i].relativedeadline;
        p[i].temparrival=p[i].arrival;
        p[i].absolutedeadline=p[i].relativedeadline+p[i].arrival;
    ...
    for(int i=0;i<n;i++)
    {
        sum+=(float)p[i].execution/p[i].relativedeadline;
    }
    cout<<sum<<endl;
    if(sum<=1)
    {
