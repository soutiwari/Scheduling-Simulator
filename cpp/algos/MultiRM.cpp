...
using namespace std;
struct process{
    int pno,arrival,execution,period;
    bool flag;
...
int main(int arg, char* argv[])
{
    ...
    for(int i=0;i<nofprocessor;i++)
    {
        proc[i].utlization=(float)(p[i].execution)/(float)(p[i].period);
    ...
    for(int i=nofprocessor;i<nofprocess;i++)
    {
        ...
        float util=(float)(p[i].execution)/(float)(p[i].period);
   ...
   struct startend st[nofprocess+1];
    for(int i=0;i<nofprocessor;i++)
    {
        ...
        for(int j=0;j<proc[i].k;j++)
        {
            ...
            plist[i][j].period=p[proc[i].index[j]].period;
    ...
    for(int l=0;l<nofprocessor;l++)
    {
        ...
        while(hyper>=clock)
        {
            if(plist[l][0].arrival<=clock)
            {
                ...
                {
                    plist[l][0].arrival=plist[l][0].arrival+plist[l][0].period;
